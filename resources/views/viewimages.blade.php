                ...
                <div class="modal-body">
                    ...
                    <form action="" method="POST" enctype="multipart/form-data">
                        ...
                        {{-- @php
                            ...
                            foreach ($arrImg as $img) {
                                echo '<img src="' . asset('storage/' . $img) . '" alt="Image" width="100">';
                                echo'<button class="btn btn-danger" onclick="confirmDelete({{ $photo->id }})">Delete</button>';
                        ...
                        @foreach ($arrImg as $img)
                            <div style="display: inline-block; margin: 10px; position: relative;">
                                <img src="{{ asset('storage/' . $img) }}" alt="Image" width="100"
                                    onclick="confirmDelete({{ $photo->id }}, '{{ $img }}')"
...
@foreach ($arrImg as $img)
    <div>
        <img src="{{ asset('storage/' . $img) }}" alt="Image" width="100">
        <button class="btn btn-danger" onclick="confirmDelete({{ $photo->id }}, '{{ $img }}')">Delete</button>
    </div>
@endforeach --}}

<script>
    function confirmDelete(photoId, imagePath) {
        if (confirm("Are you sure you want to delete this image?")) {
            $.ajax({
                url: `/delete-image/${photoId}`,
                type: 'DELETE',
                data: {
